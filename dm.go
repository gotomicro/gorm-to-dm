...
import (
	...
	"github.com/gotomicro/gorm-to-dm/clauses"
)
...
func (d Dialector) Initialize(db *gorm.DB) (err error) {
	...
	if d.Conn != nil {
		...
		u, err := url.Parse(fmt.Sprintf("dm://%s", d.DSN))
		if err != nil {
			return err
		}
		// ns lookup host
		if u.Host != "" {
			host := u.Hostname()
			port := u.Port()
			if serverListStr := u.Query().Get("serverList"); serverListStr != "" {
				// 如果是集群模式，可以在 qs 里传入 serverList，格式为 ip:port,ip:port，此时 host 代表 serverName，不进行 nslookup
				dm.ServerGroupMap.Store(host, dm.ParseServerName(host, serverListStr))
			} else {
				// 单节点模式，host 可能是 ip 或者 hostname，需要 nslookup 找到 ip，否则 dmgo 驱动里会默认为 localhost
				if ip := net.ParseIP(host); ip == nil {
					...
					u.Host = fmt.Sprintf("%s:%s", ips[0].String(), port)
				}
			}
		}
		db.ConnPool, err = sql.Open(d.DriverName, u.String())
		if err != nil {
			return err
		}
	}
	// TODO 尝试从 达梦 server 获取该 schema 下所有 table 的 schema 信息

	if err = db.Callback().Create().Replace("gorm:create", Create); err != nil {
		return
	}
	if err = db.Callback().Query().Before("gorm:query").Register("dm:sql_parser", Mysql2DM); err != nil {
		return
	}
	if err = db.Callback().Update().Before("gorm:update").Register("dm:sql_parser", Mysql2DM); err != nil {
		return
	}
	if err = db.Callback().Delete().Before("gorm:delete").Register("dm:sql_parser", Mysql2DM); err != nil {
		return
	}
	if err = db.Callback().Row().Before("gorm:row").Register("dm:sql_parser", Mysql2DM); err != nil {
		return
	}
	if err = db.Callback().Raw().Before("gorm:raw").Register("dm:sql_parser", Mysql2DM); err != nil {
		return
	}
...
func (d Dialector) QuoteTo(writer clause.Writer, str string) {
	...
	// hack 一下，如果含有 __ 则大概率 as 的，这里不做 upper 处理
...
func (d Dialector) SavePoint(tx *gorm.DB, name string) error {
	_, err := tx.ConnPool.ExecContext(tx.Statement.Context, "SAVEPOINT "+name)
	return err
}

func (d Dialector) RollbackTo(tx *gorm.DB, name string) error {
	_, err := tx.ConnPool.ExecContext(tx.Statement.Context, "ROLLBACK TO SAVEPOINT "+name)
	return err
}
